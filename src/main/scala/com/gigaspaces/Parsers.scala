...
trait Parsers[Parser[+ _]] {
  ...
  case class ParserOps[A](p: Parser[A]) {
    ...
    def <*(p2: => Parser[Any]): Parser[A] = parsers.skipR(p, p2)

    def token: Parser[A] = parsers.token(p)

    def sep(separator: Parser[Any]): Parser[List[A]] = parsers.sep(p, separator)

    def sep1(separator: Parser[Any]): Parser[List[A]] = parsers.sep1(p, separator)

    def as[B](b: B): Parser[B] = parsers.map(parsers.slice(p))(_ => b)
